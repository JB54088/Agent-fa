...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json().catch(() => ({})) as { batchSize?: number; batchName?: string };
    return NextResponse.json(await runInitialSyncBatch({ adminId, batchSize: body.batchSize, batchName: body.batchName }));
  } catch (error) {

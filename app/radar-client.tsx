...
export default function Home({ initialView = "home" }: { initialView?: RadarView } = {}) {
  ...
  return (
    <div className="app-shell">
      ...
      {profileOpen && <ProfileQuickPanel profile={profile} onClose={() => setProfileOpen(false)} onEdit={() => { setProfileOpen(false); navigate("profile"); }} onLogout={() => { setLoggedIn(false); setIsAdmin(false); setProfileOpen(false); void fetch("/api/auth/logout", { method: "POST" }).finally(() => window.location.assign("/login")); }} />}

...
await writeFile("docs/data-sources/national-directory.csv", rows(["id", "name", "category", "scope", "region_code", "region_name", "required_official_roles", "source_url", "source_domain", "normal_frequency", "active_frequency", "discovery_status", "automation_allowed", "notes"], nationalSourceDirectory.map((item) => ({ id: item.id, name: item.name, category: item.category, scope: item.scope, region_code: item.regionCode, region_name: item.regionName, required_official_roles: item.requiredOfficialRoles.join("、"), source_url: item.sourceUrl, source_domain: item.sourceDomain, normal_frequency: item.normalFrequency, active_frequency: item.activeFrequency, discovery_status: item.discoveryStatus, automation_allowed: item.automationAllowed, notes: item.notes }))));
